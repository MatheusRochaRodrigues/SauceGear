...
struct GBufferBinder {
    static void Bind(const Framebuffer& g) {
        ...
        glActiveTexture(GL_TEXTURE1); glBindTexture(GL_TEXTURE_2D, g.GetTextureByType(FramebufferTextureType::Albedo)); //HDR
        glActiveTexture(GL_TEXTURE2); glBindTexture(GL_TEXTURE_2D, g.GetTextureByType(FramebufferTextureType::Normal));
