...
struct MaterialInspector {
    ...
    // ===============================
    static bool BeginInspectorTable(const char* id)
    {
    ...
    static void EndInspectorTable()
    {
        ImGui::EndTable();
    }

    static void InspectorRow(const char* label)
    {
        ImGui::TableNextRow();
    ...
    static void DrawFieldTexture(
        ...
        ImGui::Image(id, ImVec2(48, 48), { 0,1 }, { 1,0 });
        ...
        if (tex && tex->isSolidColor) {
            ...
            case MaterialBase::ParamDef::UIType::Color: { 
                ...
                /*if (!std::holds_alternative<Color>(v.data))
                    v.data = Color{ glm::vec4(1,1,1,1) };

                Color& col = std::get<Color>(v.data);

                if (ImGui::ColorEdit4(name.c_str(), &col.value.x)) {
                    inst.dirty = true;
                }
                */
                break;
    ...
    static void Draw(MaterialInstance& inst)
    {
        ...
        if (BeginInspectorTable("MaterialInspector"))
        {
...
private:
    ...
    static void DrawParam(
        ...
        case MaterialBase::ParamDef::Vec3: {
            glm::vec3 v = std::get<glm::vec3>(value->data);
        ...
        case MaterialBase::ParamDef::Vec4: {
            glm::vec4 v = std::get<glm::vec4>(value->data);
            if (ImGui::ColorEdit4(
                "##v",
                &v.x,
                ImGuiColorEditFlags_Float |
        ...
        case MaterialBase::ParamDef::Texture:
            DrawFieldTexture(name.c_str(), *value, def, inst);
            break;
        }
