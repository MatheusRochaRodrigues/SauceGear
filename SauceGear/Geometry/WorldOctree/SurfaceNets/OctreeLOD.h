...
public:
    ...
    void Update() {
        ...
        while (!q.empty()) {
            ...
            if (
                !n->is_leaf() && (!n->isChunk() && (n->chunk == nullptr))
                && (!is_materialized(n) || /*if is above of min lod of chunks*/ n->depthLOD > sysv.get_MinChunkLod())
                )  {
                    for (auto* c : n->children) q.push(c);
            }
...
private:
    ...
    void subdivide(OctreeNode* n) {
        if (n->depthLOD <= syso.maxDepthLod || n->subdivided) return; 
        
        int   childLOD      = n->depthLOD - 1;                                  //float half          = n->size * 0.5f;
        float childOffset   = n->edge_length() * 0.5f;    //* 0.25f
