...
public:   
    static inline void aabbMesh(Entity e) {
        ...
        auto local = ComputeLocalAABB(*scn->GetComponent<MeshRenderer>(e).mesh->mesh);
    ...
    static inline AABB aabbMesh(MeshAsset& mesh, glm::mat4 m) {
        return TransformAABB(ComputeLocalAABB(mesh), m);
    }

    static inline AABB ComputeLocalAABB(const MeshAsset& mesh) {
        AABB box;
