...
public:
    ...
    static inline GLuint resolution = 2048;
}; 
...
public:
    ...
    static inline GLuint CreateDebugLayerTexture(int layer) {
    ...
    static inline Shader* GetDebugShader() {
        static Shader* debugShader = nullptr;
        if (!debugShader) debugShader = new Shader( "debugQuad.vs", "debugQuadDepth.fs" ); 
        return debugShader;
    }

    // 4️⃣ desenha a textura no quad
    static inline void DrawDebugLayer(GLuint tex2D) {
        if (!tex2D) return;
    ...
    // 4️⃣ desenha a textura no quad
    static inline void DrawDebugShadowLayerTex2D() {
        Shader* shader = GetDebugShader();
        shader->use();
