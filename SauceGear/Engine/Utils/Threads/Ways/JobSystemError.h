...
struct Job
{
    ...
    JobCounter* counter = nullptr;                  // std::atomic<int>* counter = nullptr;
};
...
private:
    ...
    void Execute(Job& job) {
        ...
        // libera data (IMPORTANTE)
        delete job.data;                //TESTEEEEEEEEEEEEEEEEE

        if (job.counter) {
            if (job.counter->value.fetch_sub(1) == 1)
            {
                // Último job terminou
                if (job.counter->onComplete) job.counter->onComplete(); 
